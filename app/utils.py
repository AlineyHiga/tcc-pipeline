...
def run_sonar_scanner(project_dir: str = ".") -> int:
    return Utils.run_sonar_scanner(project_dir)

def format_issues_for_prompt(issues) -> str:
    lines = []
    for issue in issues:
        lines.append(f"- {issue['severity']}: {issue['rule']} at {issue['component']}:{issue['line']} - {issue['message']}")
    return "\n".join(lines)

def ensure_git_branch(branch_name: str) -> None:
    Utils.create_branch_and_commit(branch_name, "temp commit")
...
def create_pull_request(title: str, body: str, head: str, base: str) -> dict:
    Utils.open_github_pr(head, title, body)
    return {"html_url": f"https://github.com/{os.environ.get('GITHUB_REPO')}/pull/1"}